...
function init() {
    const container = componentChat.container
    container.style.background = serviceStyle.vars["--background-transparent01"]
    ...
    console.style.flexDirection = 'column-reverse'
    ...
    input.style.background = serviceStyle.vars["--background-transparent03"]
    ...
    function append(str) {
        ...
        consoleSpans[spanConsoleIndex].textContent = str
        console.prepend(consoleSpans[spanConsoleIndex])
    ...
    addEventListener('keydown', ({ code }) => {
        if (code === 'Enter' && document.activeElement.tagName !== 'INPUT' && !componentChat.isLock) input.focus()
    })
    ...
    function focus() {
        clearTimeout(blurTimeout)
        container.style.background = serviceStyle.vars["--background-transparent05"]
    }
